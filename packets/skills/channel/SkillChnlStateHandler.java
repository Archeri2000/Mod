...
//Client Sided
public class SkillChnlStateHandler implements IMessageHandler<SkillChnlStatePacket, IMessage> { 
	
	
    @Override
    public IMessage onMessage(final SkillChnlStatePacket message, final MessageContext ctx) { 
        Main.proxy.getThreadFromContext(ctx).addScheduledTask(new Runnable() {
            ...
            public void run() {
            	...
            	if(Cst != null){
                	Cst.setChannelState(message.state);
            	}
            }
        });
        return null;
